...
| Radial Attention (arXiv:2506.19852) | abandoned (branch `perf/sparse-attn-experiments`) | low | regression / divergence | Static spatiotemporal-decay mask + sink at frame 0.  Paper requires 12 dense warmup steps; LTX 2.3 distilled has 11 total.  Tested: dense_steps=8 (stage 1 dense, stage 2 radial) at 384×640 → cos 0.727, identity drift / scene morphing.  Block-boundary alignment at LTX's tpf=144/240 was a real separate issue (fixed via `block_size = largest divisor of tpf ≤ 128`, gives 72/120/96 respectively).  See `PERFORMANCE_NOTES.md` "2026-05-23 perf-sparse-attn-experiments". |
| AdaSpa (arXiv:2502.21079) | abandoned (branch `perf/sparse-attn-experiments`) | medium | regression | Dynamic per-(layer, head, step) mask from LSE-cached attention statistics. Same warmup wall as radial (paper t_w=10 on 50-step models).  Tested: sparsity=0.2, warmup=4 at 384×640 → cos 0.657, +60% wall (Python-side sparse SDPA gets *slower* than dense at 80% density due to per-block dispatch tax).  See `PERFORMANCE_NOTES.md`. |
| FBCache (arXiv:2411.19108) | abandoned (branch `perf/sparse-attn-experiments`) | low | −3.5% wall, divergent output | Whole-step skip when block-0 residual delta is below threshold.  Tested at 384×640: 2 of 8 stage-1 forwards skip; stage 2 gets no skips (shape change forces cache rebuild, only 2 forwards left with deltas 0.22 / 0.28 > threshold 0.05).  Net: 8m 27s → 8m 09s = 3.5% off bakery, output is coherent + temporally stable but content-divergent from baseline at same seed.  Same-seed-different-output property breaks iterative prompt-tuning workflows; user judged 3.5% not worth it.  See `PERFORMANCE_NOTES.md`. |
| Bool-mask cast bug in `_attention_core_inline_with_mask` | fixed on branch `perf/sparse-attn-experiments` | none | none | Unconditional `mask.astype(q.dtype)` turned bool masks into 0/1 BF16 → MLX SDPA interprets as additive bias (~2.72× preference), not hard mask.  Not exercised on main since no caller passes bool, but real bug.  Fix: `if mask.dtype != mx.bool_: mask = mask.astype(q.dtype)`.  See `PERFORMANCE_NOTES.md`. |
| Path-b sparse SDPA via gather + per-Q-block dense | crossover at high sparsity only (branch) | low | density-dependent | `scripts/test_radial_sparse.py`: T=2304 0.55× (regression at any density), T=8784 1.08× at 33% density, T=14640 1.16× at 30%.  Python-loop dispatch tax dominates below 50% sparsity → distilled LTX can't reach sparsity where path-b wins.  Real Metal kernel (path-c) needed to fix; multi-week, same risk class as FA-2 dead-end. |
| dot4I8Packed Metal kernel feasibility | queued | medium | unknown | M1 silicon has the int8-dot instruction (~21 TFlops/s theoretical) but neither Apple, MLX, nor Draw Things wrote a real kernel.  Standalone bench: can a custom MSL kernel using `simd_dot` int8 intrinsics beat the BF16 7.95 TFlops/s ceiling on LTX's FF shape?  Yes → re-opens quantization (currently dead-end #2).  See `PERFORMANCE_NOTES.md` "Queued: `dot4I8Packed`". |
