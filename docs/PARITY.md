...
| **Fused interleaved RoPE** | **Guard fixed 2026-05-31** | LTX-2.3 distilled uses split RoPE, so this is not on the hot path.  The legacy/interleaved fused kernel now reads `x`, `cos`, and `sin` as FP32 before casting the final result back to the activation dtype, matching the fallback precision pattern. |
