...
// SPDX-License-Identifier: MIT
  ...
  static_assert(TQ == 1, "Lite LTX STEEL attention expects TQ=1.");
  ...
  if (!AlignQ && int(tid.x) == NQ_aligned) {
    ...
    Otile.template store_safe<T, 1, 1>(O, O_stride_t, dst_tile_dims);
  } else {
    Otile.template store<T, 1, 1>(O, O_stride_t);
  }
