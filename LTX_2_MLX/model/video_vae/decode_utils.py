"""Whole-clip VAE decode helpers, decoder-agnostic (``decoder(latent, timestep=...)``).

``decode_single_pass`` decodes the entire latent in one call (the unsafe,
seam-free path). The chunked, streaming path lives in
``tiling.decode_streaming``. Typed ``Any`` to keep this module
concrete-class-free.
"""
